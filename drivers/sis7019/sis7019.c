...
struct sis7019 {
	...
	int silence_users;
	struct snd_dma_buffer silence_buf;
};
...
static void __sis_map_silence(struct sis7019 *sis)
{
	...
	if (!sis->silence_users)
		...
		sis->silence_dma_addr = dma_map_single(&sis->pci->dev,
						...
						4096, DMA_TO_DEVICE);
#endif
		snd_dma_alloc_pages(SNDRV_DMA_TYPE_DEV, &sis->pci->dev, 4096, &sis->silence_buf);
...
static void __sis_unmap_silence(struct sis7019 *sis)
{
	...
	if (!sis->silence_users)
		...
		dma_unmap_single(&sis->pci->dev, sis->silence_dma_addr, 4096,
					DMA_TO_DEVICE);
#endif
		snd_dma_free_pages(&sis->silence_buf);
}
					...
					struct snd_pcm_substream *substream)
{
	...
	writel(sis->silence_buf.addr, play_base + SIS_PLAY_DMA_BASE);
